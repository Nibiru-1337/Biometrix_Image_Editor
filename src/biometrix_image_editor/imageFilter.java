...
protected static BufferedImage Sobel(BufferedImage img){
    ...
    for (int x = 1; x < originalCopy.getWidth()-1; x++){
        for (int y = 1; y < originalCopy.getHeight()-1; y++){
            ...
            xR = ( basicImageIO.getR(tmp[2][0]) + 2 * basicImageIO.getR(tmp[2][1])
                    + basicImageIO.getR(tmp[2][2]) ) - ( basicImageIO.getR(tmp[0][0])
                    + 2 * basicImageIO.getR(tmp[0][1]) + basicImageIO.getR(tmp[0][2]) );

            xG = ( basicImageIO.getG(tmp[2][0]) + 2 * basicImageIO.getG(tmp[2][1])
                    + basicImageIO.getG(tmp[2][2]) ) - ( basicImageIO.getG(tmp[0][0])
                    + 2 * basicImageIO.getG(tmp[0][1]) + basicImageIO.getG(tmp[0][2]) );     

            xB = ( basicImageIO.getB(tmp[2][0]) + 2 * basicImageIO.getB(tmp[2][1])
                    + basicImageIO.getB(tmp[2][2]) ) - ( basicImageIO.getB(tmp[0][0])
                    + 2 * basicImageIO.getB(tmp[0][1]) + basicImageIO.getB(tmp[0][2]) );

            yR = ( basicImageIO.getR(tmp[0][2]) + 2 * basicImageIO.getR(tmp[1][2])
                    + basicImageIO.getR(tmp[2][2]) ) - ( basicImageIO.getR(tmp[0][0])
                    + 2 * basicImageIO.getR(tmp[1][0]) + basicImageIO.getR(tmp[2][0]) );

            yG = ( basicImageIO.getG(tmp[0][2]) + 2 * basicImageIO.getG(tmp[1][2])
                    + basicImageIO.getG(tmp[2][2]) ) - ( basicImageIO.getG(tmp[0][0])
                    + 2 * basicImageIO.getG(tmp[1][0]) + basicImageIO.getG(tmp[2][0]) );               

            yB = ( basicImageIO.getB(tmp[0][2]) + 2 * basicImageIO.getB(tmp[1][2])
                    + basicImageIO.getB(tmp[2][2]) ) - ( basicImageIO.getB(tmp[0][0])
                    + 2 * basicImageIO.getB(tmp[1][0]) + basicImageIO.getB(tmp[2][0]) );        
            ...
            //set new pixel
            nPixel = basicImageIO.toRGB(basicImageIO.checkBounds(newR),
                    basicImageIO.checkBounds(newG),basicImageIO.checkBounds(newB));
